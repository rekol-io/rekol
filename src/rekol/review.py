...
configured interval. "Confirmed" reuses the ``updated`` timestamp.
"""
...
def find_overdue(
    ...
    *,
    ...
    """Return overdue durable memories as ``[{file_path, updated, age_days}]``.

    ``rows`` is ``[{file_path, updated, created}]``. A file is durable when its
    layer is in ``exempt_layers``; overdue when its ``updated`` (fallback
    ``created``) is older than ``interval_days``, or absent. Most-overdue first
    (a memory with no usable date is treated as the most overdue).
    """
    ...
    for row in rows:
        if _layer_of(row["file_path"], memory_home) not in exempt:
            continue
        ref = _as_date(row.get("updated") or row.get("created"))
        if ref is None:
            out.append(
                {"file_path": row["file_path"], "updated": row.get("updated"), "age_days": None}
            )
